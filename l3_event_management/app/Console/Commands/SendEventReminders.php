...
class SendEventReminders extends Command
{
    ...
    {
        $events = Event::with('attendees.user')->whereBetween('start_time',[now(), now()->addDay()])->get();// all the events that start in the next 24 hours
        $eventCount = $events->count();
        $eventLabel = Str::plural('event',$eventCount);
        $this->info("Found {$eventCount} {$eventLabel}");
        // $events->each(fn($event)=>$event->attendees->each(fn($attendee)=>$this->info("Nofifying the user {$attendee->user->id}")));
